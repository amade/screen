...
void LGotoPos(struct layer *l, int x, int y)
{
	...
	FOR_EACH_UNPAUSED_CANVAS(l, {
				 ...
				 continue;
				 x2 = x + cv->c_xoff;
				 y2 = y + cv->c_yoff; debug("---LGotoPos %d %d\n", x2, y2); if (x2 < cv->c_xs)
...
void LScrollH(struct layer *l, int n, int y, int xs, int xe, int bce, struct mline *ol)
{
	...
	FOR_EACH_UNPAUSED_CANVAS(l, for (vp = cv->c_vplist; vp; vp = vp->v_next) {
...
void LScrollV(struct layer *l, int n, int ys, int ye, int bce)
{
	...
	FOR_EACH_UNPAUSED_CANVAS(l, for (vp = cv->c_vplist; vp; vp = vp->v_next) {
				 ...
				 debug(" -> %d %d\n", ys2, ye2); if (ye2 - ys2 == ye - ys)
				 continue; if (n > 0) {
				 ys2 = ye2 + 1 - n; ye2 = ye + vp->v_yoff - n;}
				 else
				 {
				 ye2 = ys2 - 1 - n; ys2 = ys + vp->v_yoff - n;}
				 debug("LScrollV: - %d %d\n", ys2, ye2); if (ys2 < vp->v_ys)
				 ys2 = vp->v_ys; if (ye2 > vp->v_ye)
				 ye2 = vp->v_ye; debug("LScrollV: - %d %d\n", ys2, ye2); if (ys2 <= ye2)
