...
void MouseMode(int mode)
{
	...
	if (D_mouse != mode) {
		...
		if (mode) {
			u32_sprintf(mousebuf, "\033[?%dh", mode);
			AddStr(mousebuf);
		}
		D_mouse = mode;
	}
...
#endif
				Msg(errno, U"Error writing output to display");
	}
}
